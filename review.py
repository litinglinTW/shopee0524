...
class ShopeeCommentCrawler:
    ...
    async def fetch_comments(self, session, shopid, itemid, offset=0, limit=50):
            ...
            try:
                async with session.get(self.base_api, params=params, proxy=self.proxy, timeout=10, headers=self.headers) as resp:
                    ...
                    if status == 403:
                        ...
                        return {"error": "HTTP 403 Forbidden：可能是IP被蝦皮封鎖、Cookie失效、或Header不完整"}
                    elif status == 429:
                        logger.error("HTTP 429 Too Many Requests：被蝦皮暫時限制，請降低抓取頻率或更換IP")
                        return {"error": "HTTP 429 Too Many Requests：被蝦皮暫時限制，請降低抓取頻率或更換IP"}
                    elif status == 401:
                        logger.error("HTTP 401 Unauthorized：Cookie 無效或未登入")
                        return {"error": "HTTP 401 Unauthorized：Cookie 無效或未登入"}
                    elif status != 200:
                        logger.warning(f"HTTP error {status} for offset {offset}, attempt {attempt}/{self.retries}")
                        return {"error": f"HTTP error code {status}，請檢查網路或Cookie/Header設置"}
                    else:
                        try:
                            data = await resp.json()
                        except Exception as e:
                            logger.error(f"API回應不是有效JSON：{e}")
                            return {"error": f"API回應不是有效JSON：{e}"}
                        # 檢查API回傳內容
                        if "data" not in data or data["data"] is None:
                            text = await resp.text()
                            print("API 原始回應內容：", text)
                            logger.error(f"API原始回應內容：{text}")
                            logger.error("API回傳格式異常，可能是Header不足或Cookie失效")
                            return {"error": "API回傳格式異常，可能是Header不足或Cookie失效"}
                        if "ratings" in data["data"] and not data["data"]["ratings"]:
                            logger.info("API正常回傳但無留言，可能是商品無留言或留言未公開")
                            return {"error": "API正常回傳但無留言，可能是商品無留言或留言未公開"}
                        return data
            except Exception as e:
                text = await resp.text()
                print("API 原始回應內容（非 JSON）：", text)
                logger.error(f"API原始回應內容（非 JSON）：{text}")
        ...
        return {"error": "多次重試失敗，可能是網路問題或被蝦皮封鎖"}

    async def crawl_all_comments(self, shopid, itemid):
        async with aiohttp.ClientSession() as session:
            offset = 0
            limit = 50
            total_count = 1  # 預設 > 0，進入迴圈
            while offset < total_count:
                ...
                if not data or ("error" in data and data["error"]):
                    logger.error(f"⚠️ {data['error'] if data and 'error' in data else '蝦皮伺服器沒有回應，可能是被擋下（HTTP 錯誤或網路問題）'}")
                    break
...
async def main(product_url: str, cookie: str = "", proxy: str = None):
    ...
    comments = await crawler.crawl_all_comments(shopid, itemid)

    if not comments or len(comments) == 0:
        return None, None, "❌ 未能成功取得留言，可能是商品被擋或沒有公開留言"

    try:
        fig = crawler.analyze_comments()
        df = pd.DataFrame(crawler.comments)
        return fig, df, None  # 沒錯誤
    except Exception as e:
        logger.exception("分析過程發生錯誤")
        return None, None, f"❌ 分析留言時發生錯誤：{e}"
...
def run_review_report():
    ...
    if st.button("開始分析留言"):
        ...
        with st.spinner("分析中，請稍候..."):
            try:
                fig, df, error = asyncio.run(main(product_url, cookie, proxy))
                if error:
